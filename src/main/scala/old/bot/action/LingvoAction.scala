...
import old.bot.{Ignore, Lingvo, Outcome}
import org.slf4j.LoggerFactory
import old.persistence.Memory
import old.persistence.model.Query
...
class LingvoAction(
  ...
  lingvoService: old.lingvo.LingvoService
)(
  ...
  override def run(command: Lingvo): Future[Outcome] = {
    ...
    future
      .recoverWith {case ex: Throwable =>
        log.error(s"Cannot process old.lingvo [$text]", ex)
        telegram
