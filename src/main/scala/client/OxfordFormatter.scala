...
object OxfordFormatter {

  def format(oxfordResponse: OxfordDefinition): String = {
  ...
  private def formatSense(sense: Sense, index: Option[Int] = None, parentIndex: Option[Int] = None): String = {
    ...
    val maybeCrossReferences =
      sense.crossReferences
        .map(_.map(reference => s"_${reference.`type`}_ *${reference.text}* ").mkString(", "))
