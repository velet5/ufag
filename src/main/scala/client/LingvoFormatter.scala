...
import model.client.LingvoArticle.{Node, NodeType}
import util.text.RomanNumbers

object LingvoFormatter {

  def format(articles: List[LingvoArticle]): String =
    processArticle(articles)

  // under the hood

  private def processArticle(articleModels: List[LingvoArticle]): String = {
  ...
  private def convertModel(model: LingvoArticle): Part =
    PartList(list = model.body.map(convertNode), tpe = 0)

  private def convertNode(node: Node): Part =
    node.node match {
  ...
  private def collectText(node: Node)(implicit sb: StringBuilder): Unit = {
  ...
  private def collectText(nodes: Seq[Node])(implicit sb: StringBuilder): Unit = {
